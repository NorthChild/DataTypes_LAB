...
namespace DataTypes_Test
{
    public class CalculatorTests
    {
        ...
        {
            Assert.That(() => IntegerCalc.Add(int.MaxValue, 3), Throws.TypeOf<Exception>().With.Message.Contains("Invalid Input"));
        }

        [Test]
        public void WhenGivenTwoVeryNegativeIntegers_Add_ThrowsAnException()
        {
            Assert.That(() => IntegerCalc.Add(int.MinValue, -3), Throws.TypeOf<Exception>().With.Message.Contains("Invalid Input"));
        }
        ...
        {
            Assert.That(() => IntegerCalc.Subtract(int.MaxValue, -3), Throws.TypeOf<Exception>());
        }

        [Test]
        public void WhenGivingMoreLargeOperands_Subtract_ThrowsAnException()
        {
            Assert.That(() => IntegerCalc.Subtract(int.MinValue, 3), Throws.TypeOf<Exception>());
        }
